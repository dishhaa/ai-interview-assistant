...
export default function ChatWindow({ candidateId }) {
  ...
  const handleSubmitAnswer = () => {
    ...
    const score = evaluateAnswer(userAnswer, currentQ.question);

    const newChat = [
      ...chat,
      { role: "bot", text: currentQ.question },
    ...
    <Card title={`Interview for ${candidate.name}`} style={{ marginTop: 20 }}>
      {!interviewDone ? (
        <>
          ...
          />
          <Button type="primary" onClick={handleSubmitAnswer} style={{ marginTop: 10 }}>
            Submit Answer
          </Button>
        </>
      ) : (
